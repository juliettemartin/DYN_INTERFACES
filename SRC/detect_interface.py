...
def args_gestion():
        ...
        parser.add_argument("--method", metavar = "<str>", help = "[ASA]. Not mandatory. Detection method used: ASA (default) or Levy", default = "ASA",required=False)
...
if ARGS.method=="Levy":
        with open('result.txt','w') as f_result:
                for res in res_list:
                        reformatted_res=str.split(res)[1]+','+str.split(res)[0]+' '+str.split(res)[2]+' '
                        if unbound_asa[res] == bound_asa[res]:
                                if bound_asa[res]< 25.0:
                                        f_result.write(reformatted_res+',interior\n')
                                else:       
                                        f_result.write(reformatted_res+',surface\n')

                        else:
                                if unbound_asa[res]< 25.0:
                                        f_result.write(reformatted_res+',support\n')
                                if bound_asa[res] > 25.0:
                                        f_result.write(reformatted_res+',rim\n')
                                if unbound_asa[res] > 25.0 and bound_asa[res]<25.0:
                                        f_result.write(reformatted_res+',core\n')
