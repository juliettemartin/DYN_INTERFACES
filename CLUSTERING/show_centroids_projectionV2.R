...
centroids_v2=scan(input[3],what="e")

N=length(centroids_v2)
...
pdf(width=4,height=3,file="Time_line.pdf")
ggplot(my_data,aes(x,y,col=as.numeric(label)))+geom_path()+geom_point(data=my_data[1,],aes(x,y),col="red",size=2,shape=3,stroke=2)+geom_point(data=my_data[my_data$centroids_v2=="YES",],aes(x,y), col="black",size=2,shape=1,stroke=2)+theme_bw()+labs(col="time")+xlab(paste("PC1, %var=",format(var_expl[1],digits=3)))+ylab(paste("PC2, %var=",format(var_expl[2],digits=3)))+scale_colour_gradient(low="yellow",high="blue")
dev.off()

pdf(width=4,height=3,file="Time_line_no_centroid.pdf")
ggplot(my_data,aes(x,y,col=as.numeric(label)))+geom_path()+geom_point(data=my_data[1,],aes(x,y),col="red",size=2,shape=3,stroke=2)+theme_bw()+labs(col="time")+xlab(paste("PC1, %var=",format(var_expl[1],digits=3)))+ylab(paste("PC2, %var=",format(var_expl[2],digits=3)))+scale_colour_gradient(low="orange",high="black")
dev.off()
